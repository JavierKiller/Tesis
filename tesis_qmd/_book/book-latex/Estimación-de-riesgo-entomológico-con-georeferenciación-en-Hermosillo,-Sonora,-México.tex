...
agente infeccioso. A este tipo de artrópodos se le denomina ``vector''.
Este vector actúa como hospedero y/o transmisor de la enfermedad,
durante su ciclo de vida tienen diferentes hospederos periodo en el cual
adquiere la enfermedad y puede transmitirla a hospederos vertebrados
como los humanos. Algunos ejemplos de este tipo de enfermedades son:
paludismo, dengue, leishmaniasis, oncocercosis, tripanosomiasis,
rickettsiosis, fiebre del oeste del Nilo, fiebre chikungunya,
erliquiosis, anaplasmosis y otras arbovirosis. Las ETV representan un
problema de salud pública en México. Se estima que donde se localiza la
mayor parte de centros agrícolas, ganaderos, industriales, pesqueros,
petroleros y turísticos, es decir; en cerca de 60 \% del territorio
nacional, se tienen condiciones ambientales que favorecen su transmisión
. Entre ellas, el dengue representa una grave amenaza, con brotes
recurrentes en diversas regiones del país. Para prevenir y controlar
eficazmente las enfermedades antes mencionadas, en México existen los
programas de vigilancia entomológica y control integral de enfermedades
transmitidas por vectores a nivel federal y estatal.

\subsection{Vigilancia
Epidemiológica:}\label{vigilancia-epidemioluxf3gica}

La vigilancia epidemiológica del dengue comprende, recopilar, analizar y
difundir sistemáticamente información sobre la enfermedad. Sus objetivos
incluyen la detección temprana de brotes, la medición de la carga de la
enfermedad, el seguimiento de las tendencias, la evaluación del impacto
social y económico. Asimismo ayuda en la asignación de recursos,
planificación de acciones preventivas y de contención. Por ello, la
vigilancia epidemiológica es esencial para el control y la gestión
eficaz contra la enfermedad.

\subsection{Índices Estegomía}\label{uxedndices-estegomuxeda}

Para evaluar la presencia y densidad de los mosquitos Aedes, los
...
entomológico con respecto a criaderos potenciales y criaderos habitados
por larvas o pupas del mosquito Aedes a través del muestreo en campo. El
muestreo debe ser uniforme en el área de estudio y la unidad de muestreo
básica es la casa o localidad (CENAPRECE 2024). Al muestrear, el
objetivo es observar sistemáticamente contenedores de almacenamiento de
agua. Los contenedores se examinan para detectar la presencia de larvas
y pupas de mosquitos.

La Organización Mundial de la Salud (OMS)(Organization 2016) recomienda
el uso de los índices estegomía para evaluar la presencia y la densidad
de criaderos de mosquitos en las comunidades. Los índices utilizados son
el índice de casa positiva (HI), el índice de contenedor positivo y el
índice de Breteau. En el estado de Sonora el ``Programa de Acción
Específico de Enfermedades Transmitidas por Vector'' (PAE-ETV) usa estos
índices para ayudar a determinar las acciones que mitigan el riesgo
entomológico. A continuación, presento los índices de estegomía
empleados por el PAE-ETV.

\subsubsection*{índice de casa positiva
(HI)}\label{uxedndice-de-casa-positiva-hi}
\addcontentsline{toc}{subsubsection}{índice de casa positiva (HI)}

El índice de casa positiva (HI) es definido como la proporción de casas
infestadas con larvas o pupas. La OMS (Organization 2016) recomienda
usar la siguiente expresión para su cálculo (Garjito et al. 2020).

\begin{equation}\phantomsection\label{eq-hi-index}{         
  HI = \frac{
...
infestación de casas. Además, permite evaluar la presencia, y estimar la
distribución del mosquito. Este índice considera a las casas como unidad
de observación, no toma en cuenta la cantidad de contenedores positivos
ni la productividad de esos contenedores.
...
Dentro de un área dada, el cálculo de este índice asume la misma
probabilidad de transmisión del virus. De esta manera, si calculamos el
índice HI en una muestra de un sector se puede inferir para todo el
sector. La OMS considera en riesgo entomológico cuando al menos uno de
los índices de estegomía es igual o mayor a 5(Organization 2016)

\subsubsection*{índice de contenedor positivo
(CI)}\label{uxedndice-de-contenedor-positivo-ci}
\addcontentsline{toc}{subsubsection}{índice de contenedor positivo (CI)}
...
\begin{equation}\phantomsection\label{eq-ci-index}{         
  CI = \frac{
...
área estudiada. Este índice ayuda a evaluar la densidad de criaderos en
la zona. Por ello, proporciona información para identificar las áreas
con mayor riesgo de infestación. Por lo tanto, en caso de que el índice
CI sea bajo y el HI sea alto, se infiere que el riesgo es mínimo por
cada casa muestreada, pero está muy disperso; Lo cual sugiere el
realizar acciones de control y prevención en toda el área de estudio
(sector, colonia o localidad).

El clasificar el tipo de contenedor permite elegir la estrategia
específica para las condiciones particulares del área de estudio, como
...
de ser necesario. Otra estrategia sería el uso de químicos larvicidas
como apoyo al control físico.

En caso de que índice CI es alto y el HI sea bajo, esto indica que el
riesgo está concentrado en pocas casas infestadas, por lo que se pueden
enfocar las acciones en esa zona específica. Los índices son
complementarios y se necesitan ambos para identificar correctamente el
riesgo entomológico y tomar medidas efectivas para combatirlo.

\subsubsection*{índice de Breteau (BI)}\label{uxedndice-de-breteau-bi}
\addcontentsline{toc}{subsubsection}{índice de Breteau (BI)}

El índice Breteau (BI) describe número de recipientes positivos por cada
100 casas inspeccionadas.

\begin{equation}\phantomsection\label{eq-bi-index}{         
  BI = \frac{
   \text{Número de contenedores infestados}
  }{
    \text{Número de casas inspeccionadas}
...
positivos y el número de casas inspeccionadas. Cunado los contenedores
positivos tienden a ser uno por casa positiva el BI valdrá igual al HI.
En el caso que el BI salga alto pese a que los otros dos índices sean
bajos, esto indica una alta concentración de recipientes por casa aunque
la proporción de casas positivas sea baja y la proporción de recipientes
positivos también sea baja, en este caso el riesgo es que mas
recipientes. El BI evalúa la distribución espacial de los criaderos y
...
criadero De esta forma, el personal del programa de vectores se encarga
de revisar, colectar y contabilizar los huevecillos de los mosquitos y
realizar una comparación de la densidad. \\
...
de viviendas para identificar las especies transmisoras de ETV, asimismo
...
mencionadas en las tablas Tabla 1.1 y Tabla 1.2. Esto permite al
...
\includegraphics{imagenes/plataforma.gif}
...
{\centering \includegraphics{imagenes/TDD.jpg}
...
en R. Esta metodología permite tener menos errores, crear código más
robusto, legible y eficiente, facilitando la colaboración y el
mantenimiento a largo plazo de los proyectos.

El paquete de R del presente trabajo constituye una valiosa herramienta
para la vigilancia entomológica del dengue. Su uso puede contribuir a
mejorar la toma de decisiones en materia de prevención y control de la
enfermedad, fortaleciendo las estrategias de salud pública en México.
...
3 de ellas se dividen en dos por lo que resultan en 19 variables de
interés.

\section{descripcion de la base de datos de estudio Entomologico de fase
larvar y
pupal(EE)}\label{descripcion-de-la-base-de-datos-de-estudio-entomologico-de-fase-larvar-y-pupalee}
...
(https://geosis.mx/Aplicaciones/EntomologiayControlIntegral/). Este
formato requiere ajustes para leerlo en el programa de R. Además, hay
inconsistencia en el tipo de dato de algunas variables de interés por lo
que se crea una función para cambia las al formato adecuado que le
corresponde a cada una para poder trabajar con los datos

Tiene 121 variables de las cuales 16 son de interés para los cálculos de
índices de estegomía. En general el titulo de las variables tiene un
problema para leerse por ello se cambió los espacios entre palabras por
un guion bajo(\_) en los títulos de cada variable.

\subsection*{variables de interes}\label{variables-de-interes}
\addcontentsline{toc}{subsection}{variables de interes}

\subsubsection*{Variable para determinar el tipo de estudio
entomológico:}\label{variable-para-determinar-el-tipo-de-estudio-entomoluxf3gico}
\addcontentsline{toc}{subsubsection}{Variable para determinar el tipo de
estudio entomológico:}

Tipo de Estudio es el nombre de una variable que divide todos los datos
de esta actividad en 2 tipos, el primer tipo es ``encuesta'' que
significa que los datos se tomaron antes de realizar acciones de
``control larvario (CL)'' en el área donde se hiso el estudio
entomológico. El segundo tipo es ``verificación'' que indica que los
datos de ese estudio entomológico se hicieron después de realizar
acciones de CL. De tal manera se pueden comparar los datos antes y
después en un área determinada y evaluar si las intervenciones del
PAE-ETV al mitigado el riesgo entomológico o si requieren realizar más
acciones.

\subsection*{Variables para la
georreferenciación:}\label{variables-para-la-georreferenciaciuxf3n}
\addcontentsline{toc}{subsection}{Variables para la georreferenciación:}

Estas variables son las que se utilizan para determinar un lugar en el
espacio donde se calcula un riesgo entomólogo. a estas variables se les
asigno el formato de Carácter Estas variables son cuatro: Jurisdicción,
Municipio, Localidad y Sector, Cada una es parte de la anterior, el
estado de Sonora esta dividido en 6, jurisdicciones (ahora llamados
distritos) y cada jurisdicción tiene determinados municipios que la
conforman, a su vez cada municipio tiene localidades que pueden ser
desde ciudades a ejidos. En el caso de sectores cuanto mas grande la
localidad como una ciudad puede tener hasta cientos de sectores que la
conformen, pero cuando son lugares pequeños como un pueblo o ejido
tienen pocos sectores (de uno a 10). Los sectores corresponden a un
formato de división geográfica que usa el Instituto Nacional Electoral
(INE). Las primeras tres variables antes mencionadas tienen un numero
antes del nombre que corresponde al lugar (que numero de jurisdicción
es, que numero de municipio es y que numero de localidad es,
respectivamente), para evitar confusiones en la selección de datos
geográficos separamos este número del resto del nombre y lo colocamos en
variables independiente, estas tres nuevas variables son:
Clave\_Jurisdicción, Clave\_Municipio y Clave\_Localidad

\subsection*{Variables de intervalos de
tiempo:}\label{variables-de-intervalos-de-tiempo}
\addcontentsline{toc}{subsection}{Variables de intervalos de tiempo:}

Se selecciono dos variables para elegir el tiempo la primera es Fecha de
Inicio la cual viene en formato de carácter y se cambia a Tipo de dato
POSIX para poder usarla como fecha. Esta es la fecha en que se realiza
el EE y se puede comparar entre lugares o en el mismo lugar en
diferentes tiempos o en tiempos cercanos en el mismo lugar con diferente
tipo de estudio la segunda variable es Semana Epidemiológica la cual se
usa de forma preestablecida, en secretaria de salud y el PAE-ETV reporta
usando como unidad de tiempo el número de semana epidemiológica

\subsection*{Variables para índices de
estegomía:}\label{variables-para-uxedndices-de-estegomuxeda}
\addcontentsline{toc}{subsection}{Variables para índices de estegomía:}

Para calcular los índices de estegomía se utilizan seis variables \#(o
las primeras 4 solamente) Las variables de Casas Revisadas y Casas
Positivas se utilizan para calcular el IH. El Total de Recipientes con
Agua, y Total de Recipientes Positivos se utilizan para calcular el IC.
(El Total de Recipientes Positivos a Pupas, y No.~Total, de Pupas en
Recipientes, se utilizan para calcular índices de contenedor positiva a
pupa, porcentaje de pupas en los contenedores positivos) Otras variables
de interés son la tipología de recipiente positivo que consiste en
clasificar en tres grupos diferentes a los contenedores según la
estrategia de control que se ejecuta en dicho contenedor. Estas tres
variables son: Recipientes Tratables, Recipientes Controlables y
Recipientes Eliminables. Cuando se determina un riesgo entomológico alto
se hace un análisis de la tipología de contenedor para escoger la
estrategia ha emplear

\section{Georeferenciacion de las actividades en (datos Archivos
shapefile de los sectores de todo) el estado de
Sonora}\label{georeferenciacion-de-las-actividades-en-datos-archivos-shapefile-de-los-sectores-de-todo-el-estado-de-sonora}

Para poder georefereciar los indicadores de EE en un mapa se utilizan
los datos de Sector para definir en un mapa del estado de Sonora, esto
se hace con un archivo shapefile que contiene todos los sectores del
estado de sonora, este archivo fue proporcionado por el PAE-ETV.

\section{Shapefile de los Sectores de
Sonora}\label{shapefile-de-los-sectores-de-sonora}
...
\bibitem[\citeproctext]{ref-gobmx2023}
CENAPRECE. 2024. {``DIRECCIÓN DEL PROGRAMA DE ENFERMEDADES TRANSMITIDAS
POR VECTORES.''}
